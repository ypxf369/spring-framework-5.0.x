...
public class AnnotatedBeanDefinitionReader {
	...
	<T> void doRegisterBean(Class<T> annotatedClass, @Nullable Supplier<T> instanceSupplier, @Nullable String name,
		...
		/**
		 * 判断这个了是否需要跳过解析
		 * 通过代码可以知道spring判断是否要跳过解析，主要判断类有没有加注解
		 */
		...
		/**
		 * 处理类当中的通用注解
		 * 分析源码可以知道他主要处理 Lazy DependsOn Role等注解
		 ...
		 * 这里需要注意的是：
		 * byName和qualifiers这个变量是Annotation类型的数组，里面不仅仅存Qualifier注解
		 * 理论上里面存的是一切注解，所以可以看到下面的代码spring去循环了这个数组
		 * 然后依次判断了注解当中是否包含了Primary，时候包含了Lazy。
		 */
		if (qualifiers != null) {
